function [x_est, choice] = topology_coupled_partition(y, coeffs, fns, noise, dk, M, B)
...
for t=2:T
    ...
    for j = 1:dy
        ...
        if (length(states_idx) >= dk)
            ...
            wH = rowH./sum(rowH);
            % If all the left weights are 0s, sample uniformly
            if (sum(rowH) == 0)
                len = length(states_idx);
                k = datasample(states_idx, dk, 'Weights', ones(1,len)./len);
            else
                k = datasample(states_idx, dk, 'Weights',wH);
            end
        else
            k = states_idx;
        end
